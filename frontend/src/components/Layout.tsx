...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="h-screen bg-[#FBF9FE] flex overflow-hidden">
      {denied && (
      ...
      <aside className="w-48 bg-white flex flex-col shrink-0 h-screen sticky top-0 shadow-lg">
        <div className="p-5 border-b border-[#E6E2F2] text-center">
          <h1 className="text-sm font-bold text-[#5A5377] mb-3 tracking-wide">EXPEDIENTES SBJ</h1>
          <img src="/logo_sbj.png" alt="Logo SBJ Cirugias" className="w-28 h-auto mx-auto" />
        </div>
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          {navSections.map((section) => (
            <div key={section.title} className="mb-2">
              <p className="px-3 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-widest text-[#A9A3C7]">
                {section.title}
              </p>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-200 relative ${
                      isActive
                        ? 'bg-[#EEEAF8] text-[#5A5377]'
                        : 'text-[#7C7694] hover:text-[#5A5377] hover:bg-[#FBF9FE]'
                    }`}
                  >
                    {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-[#C3BCE3] rounded-full" />}
        ...
        <div className="p-4 border-t border-[#E6E2F2]">
          <button
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-[#5A5377] truncate">
                {(() => {
          ...
          <button
            onClick={logout}
            className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#7C7694] hover:text-red-500 hover:bg-[#FBF9FE] rounded-lg transition-colors duration-200"
          >
            <LogOut size={15} />
            Cerrar sesión
          </button>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="bg-gradient-to-r from-[#8A82BC] via-[#9B94C9] to-[#C3BCE3] px-12 py-1.5 flex items-center shadow-md">
        ...
        <footer className="pt-[7px] text-center text-xs text-[#8D87A8]">
