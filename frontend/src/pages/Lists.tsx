  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-rose-900">Listas Personalizables</h1>
          <p className="text-sm text-rose-500 mt-1">Administra las listas de registro del sistema</p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {lists.map((list) => (
          <div key={list.id} className="bg-white/90 backdrop-blur-sm rounded-xl shadow-sm border border-rose-100 p-6 hover:shadow-md hover:border-rose-200 transition-all duration-200">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-2">
                <h3 className="font-semibold text-rose-900">{list.name}</h3>
                {list.is_system && (
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className="p-1.5 hover:bg-rose-50 rounded-lg text-rose-500 transition-all duration-200 hover:scale-110 active:scale-95"
                  title="Ver registros"
                >
                  <Eye size={16} />
                </button>
            ...
            {list.description && (
              <p className="text-sm text-rose-500 mb-3">{list.description}</p>
            )}
            <div className="flex flex-wrap gap-1 mb-3">
              {list.columns_config.map((col, i) => (
                <span key={i} className="px-2 py-0.5 bg-rose-100 text-rose-600 rounded-full text-xs">
                  {col.label}
                </span>
              ))}
            </div>
            <button
              onClick={() => navigate(`/lists/${list.id}`)}
              className="text-sm text-rose-500 hover:text-rose-700 font-medium transition-colors duration-200"
            >
              Ver registros →
            </button>
          </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-rose-900/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white/90 backdrop-blur-sm rounded-xl px-5 py-3 w-[95vw] max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <h2 className="text-lg font-bold mb-4 text-rose-900">Nueva Lista Personalizable</h2>
            <div className="space-y-3">
              ...
              <div className="border border-rose-100 rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-rose-700">Columnas</span>
                  <button onClick={addColumn} className="text-xs text-rose-500 hover:text-rose-700 transition-colors duration-200">
                    + Agregar columna
                  </button>
                </div>
                {form.columns.map((col, idx) => (
                  <div key={idx} className="flex gap-2 mb-2">
                    <input
                      ...
                      className="flex-1 px-2 py-1.5 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-rose-300/30 focus:border-rose-300 transition-all duration-200"
                    />
                    <select
                      value={col.type}
                      onChange={(e) => updateColumn(idx, 'type', e.target.value)}
                      className="px-2 py-1.5 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-rose-300/30 focus:border-rose-300 transition-all duration-200"
                    >
