  ...
  return (
    <div className="h-full flex flex-col gap-4">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="font-serif text-2xl font-bold text-[#5A5377]">Listas Personalizables</h1>
        </div>
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1.5 bg-white border border-[#E6E2F2] text-slate-700 px-4 py-2 rounded-xl hover:bg-indigo-50 shadow-sm hover:shadow-md transition-all duration-200  text-sm font-medium"
          >
            <Eye size={16} />
            Ver registros
          </button>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 pr-1">
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {lists.map((list) => (
          <div key={list.id} className="bg-white rounded-xl shadow-sm border border-[#E6E2F2] p-6 hover:shadow-md hover:border-[#E6E2F2] transition-all duration-200">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className="p-1.5 hover:bg-indigo-50 rounded-lg text-slate-500 transition-all duration-200 hover:scale-110 active:scale-95"
                  title="Ver registros"
                >
                  <Eye size={16} />
                </button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-xl px-5 py-3 w-[95vw] max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <h2 className="font-serif text-lg font-bold mb-4 text-[#5A5377]">Nueva Lista Personalizable</h2>
            <div className="space-y-3">
              ...
              <input
                ...
                className="w-full px-3 py-2.5 border border-[#E6E2F2] rounded-xl text-sm focus:ring-2 focus:ring-slate-300/30 focus:border-slate-400 transition-all duration-200"
              />
              <div className="border border-[#E6E2F2] rounded-xl p-4">
                ...
                {form.columns.map((col, idx) => (
                  <div key={idx} className="flex gap-2 mb-2">
                    <input
                      ...
                      className="flex-1 px-2 py-1.5 border border-[#E6E2F2] rounded-lg text-sm focus:ring-2 focus:ring-slate-300/30 focus:border-slate-400 transition-all duration-200"
                    />
                    <select
                      value={col.type}
                      onChange={(e) => updateColumn(idx, 'type', e.target.value)}
                      className="px-2 py-1.5 border border-[#E6E2F2] rounded-lg text-sm focus:ring-2 focus:ring-slate-300/30 focus:border-slate-400 transition-all duration-200"
                    >
