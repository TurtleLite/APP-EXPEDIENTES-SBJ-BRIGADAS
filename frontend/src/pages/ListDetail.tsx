  ...
  return (
    <div className="h-full flex flex-col gap-4">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="font-serif text-2xl font-bold text-[#5A5377]">{list?.name || 'Cargando...'}</h1>
        ...
        <div className="flex gap-2">
          {user?.role !== 'direccion' && (list?.is_system ? (
            <button
              ...
              className="flex items-center gap-1.5 bg-[#9B94C9] text-white px-5 py-2.5 rounded-xl hover:bg-[#8A82BC] shadow-sm hover:shadow-md transition-all duration-200  text-sm font-medium"
            >
              <Stethoscope size={16} />
              Nuevo
            </button>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-[#E6E2F2] flex flex-col min-h-0 flex-1 transition-shadow duration-200 hover:shadow-md">
        <div className="p-3 border-b border-[#E6E2F2] space-y-2.5 shrink-0 bg-indigo-50/30">
        <div className="flex gap-2.5 flex-wrap">
              <select
                value={searchField}
                onChange={(e) => setSearchField(e.target.value)}
                className="px-3 py-2 border border-[#E6E2F2] rounded-xl text-sm bg-white focus:ring-2 focus:ring-slate-300/30 focus:border-slate-400 transition-all duration-200"
              >
              ...
              {selectedIds.size > 0 && (
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 bg-[#9B94C9] text-white rounded-xl hover:bg-[#8A82BC] shadow-sm hover:shadow-md transition-all duration-200  text-sm font-medium"
                  >
                    <Download size={16} />
          ...
          <table className="w-full">
            ...
            <tbody>
              {records.map((record, idx) => (
                <tr key={record.id} className={`border-b border-slate-100 transition-all duration-150 hover:bg-slate-100/50 ${selectedIds.has(record.id) ? 'bg-indigo-50/30' : idx % 2 === 0 ? 'bg-white' : 'bg-indigo-50/20'}`}>
                  {list?.is_system && (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-xl px-5 py-3 w-[95vw] max-w-5xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
            <h2 className="font-serif text-lg font-bold mb-4 shrink-0 text-[#5A5377]">
            ...
            <div className="flex justify-end gap-2 mt-4 shrink-0">
              ...
              <button onClick={handleSaveRecord} className="px-4 py-2 text-sm bg-[#9B94C9] text-white rounded-xl hover:bg-[#8A82BC] shadow-sm hover:shadow-md transition-all duration-200 font-medium">
