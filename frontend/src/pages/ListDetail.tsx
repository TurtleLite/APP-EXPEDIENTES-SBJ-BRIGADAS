  ...
  return (
    <div className="h-full flex flex-col gap-4">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <button onClick={() => navigate('/lists')} className="inline-flex items-center gap-1 text-sm text-rose-500 hover:text-rose-700 mb-1 transition-colors duration-200">
            <ArrowLeft size={15} />
            Volver a listas
          </button>
          <h1 className="text-2xl font-bold text-rose-900">{list?.name || 'Cargando...'}</h1>
          {list?.description && <p className="text-sm text-rose-500 mt-1">{list.description}</p>}
        </div>
        <div className="flex gap-2">
          {list?.is_system ? (
            <button
              onClick={() => setShowExpedienteForm(true)}
              className="flex items-center gap-1.5 bg-gradient-to-r from-rose-400 to-pink-400 text-white px-5 py-2.5 rounded-xl hover:from-rose-500 hover:to-pink-500 shadow-sm hover:shadow-md transition-all duration-200 hover:scale-[1.03] active:scale-[0.97] text-sm font-medium"
            >
              <Stethoscope size={16} />
              Nuevo
            </button>
      ...
      <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-sm border border-rose-100 overflow-hidden flex flex-col min-h-0 flex-1 transition-shadow duration-200 hover:shadow-md">
        <div className="p-3 border-b border-rose-100 space-y-2.5 shrink-0 bg-rose-50/30">
        <div className="flex gap-2.5 flex-wrap">
              <select
                value={searchField}
                onChange={(e) => setSearchField(e.target.value)}
                className="px-3 py-2 border border-slate-200 rounded-xl text-sm bg-white focus:ring-2 focus:ring-rose-300/30 focus:border-rose-300 transition-all duration-200"
              >
              ...
              <div className="relative flex-1 min-w-[180px]">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-xl text-sm bg-white focus:ring-2 focus:ring-rose-300/30 focus:border-rose-300 transition-all duration-200"
                />
              </div>
            </div>
            {list?.is_system && (
              ...
              {selectedIds.size > 0 && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-teal-300 to-emerald-300 text-white rounded-xl hover:from-teal-400 hover:to-emerald-400 shadow-sm hover:shadow-md transition-all duration-200 hover:scale-[1.03] active:scale-[0.97] text-sm font-medium"
                >
                  <Download size={16} />
          ...
          <table className="w-full">
              ...
              <tr className="bg-rose-50/50 border-b border-rose-100">
                {list?.is_system && (
                  <th className="w-10 px-3 py-4">
                    <button onClick={toggleSelectAll} className="text-rose-400 hover:text-rose-500 transition-colors duration-200">
                ...
                {list?.columns_config.filter(c => RECORD_COLUMNS.includes(c.key)).map((col) => (
                  <th key={col.key} className="text-left px-6 py-4 text-xs font-semibold text-rose-400 uppercase tracking-wider">
                    {col.label}
                  </th>
                ))}
            ...
            <tbody>
              {records.map((record, idx) => (
                <tr key={record.id} className={`border-b border-rose-50 transition-all duration-150 hover:bg-rose-50/50 ${selectedIds.has(record.id) ? 'bg-teal-50/30' : idx % 2 === 0 ? 'bg-white' : 'bg-rose-50/20'}`}>
                  {list?.is_system && (
                    <td className="w-10 px-3 py-4">
                      <button onClick={() => toggleSelect(record.id)} className="text-rose-300 hover:text-teal-500 transition-colors duration-200">
                        {selectedIds.has(record.id) ? <CheckSquare size={16} className="text-teal-500" /> : <Square size={16} />}
                      </button>
                    </td>
                  )}
                  {list?.columns_config.filter(c => RECORD_COLUMNS.includes(c.key)).map((col) => (
                    <td key={col.key} className="px-6 py-4 text-sm text-rose-700">
                      {col.key === 'telefono'
                        ? [record.data.telefono, record.data.telefono2, record.data.telefono3]
                            .filter(Boolean)
                            .join(' / ') || <span className="text-rose-300">-</span>
                        : record.data[col.key] || <span className="text-rose-300">-</span>
                      }
                    </td>
                  ))}
                  {(user?.role === 'admin' || (user?.role === 'direccion' && !list?.is_system)) && (
                    <td className="px-6 py-4 text-right">
                      {list?.is_system && (
                        <button
                          ...
                          className="p-1.5 hover:bg-emerald-50 rounded-lg transition-all duration-200 hover:scale-110 active:scale-95"
                          title="Exportar"
                        >
                      ...
                      <button
                        ...
                        className="p-1.5 hover:bg-rose-100 rounded-lg transition-all duration-200 hover:scale-110 active:scale-95"
                      >
                        <Pencil size={15} className="text-rose-500" />
                      </button>
                      <button
                        onClick={() => handleDeleteRecord(record.id)}
                        className="p-1.5 hover:bg-rose-100 rounded-lg transition-all duration-200 hover:scale-110 active:scale-95 ml-1"
                      >
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      {(!list?.is_system || record.created_by === user.id) && (
                        <button
                          ...
                          className="p-1.5 hover:bg-cyan-100 rounded-lg transition-all duration-200 hover:scale-110 active:scale-95"
                        >
              ...
              {records.length === 0 && (
                <tr>
                  <td colSpan={100} className="px-4 py-12 text-center text-rose-400 text-sm">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-rose-900/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white/90 backdrop-blur-sm rounded-xl px-5 py-3 w-[95vw] max-w-5xl flex flex-col shadow-2xl">
            <h2 className="text-lg font-bold mb-4 shrink-0 text-rose-900">
              {editingRecord ? 'Editar Registro' : 'Nuevo Registro'}
            </h2>
            <div className="space-y-3 overflow-y-auto">
              {list?.columns_config.map((col) => (
                <div key={col.key}>
                  <label className="block text-sm font-medium text-rose-700 mb-1">{col.label}</label>
                  {col.type === 'date' ? (
                    <input
                      type="date"
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => { setShowModal(false); setEditingRecord(null) }} className="px-4 py-2 text-sm text-rose-600 hover:bg-rose-50 rounded-xl transition-all duration-200">
                Cancelar
              </button>
              <button onClick={handleSaveRecord} className="px-4 py-2 text-sm bg-gradient-to-r from-rose-400 to-pink-400 text-white rounded-xl hover:from-rose-500 hover:to-pink-500 shadow-sm hover:shadow-md transition-all duration-200 font-medium">
