...
const RECORD_COLUMNS = ['nombre', 'edad', 'diagnostico', 'perfil', 'domicilio', 'telefono', 'telefono2', 'telefono3', 'albergue', 'nombre_medico']
  ...
  return (
    <div className="h-full flex flex-col gap-4">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <button onClick={() => navigate('/lists')} className="inline-flex items-center gap-1 text-sm text-cyan-600 hover:text-cyan-800 mb-1 transition-colors duration-200">
            <ArrowLeft size={15} />
            Volver a listas
          </button>
        ...
        <div className="flex gap-2">
          {list?.is_system ? (
            <button
              ...
              className="flex items-center gap-1.5 bg-gradient-to-r from-cyan-600 to-blue-600 text-white px-5 py-2.5 rounded-xl hover:from-cyan-700 hover:to-blue-700 shadow-sm hover:shadow-md transition-all duration-200 hover:scale-[1.03] active:scale-[0.97] text-sm font-medium"
            >
              <Stethoscope size={16} />
              Nuevo
            </button>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col min-h-0 flex-1 transition-shadow duration-200 hover:shadow-md">
        <div className="p-3 border-b border-slate-100 space-y-2.5 shrink-0 bg-slate-50/50">
        <div className="flex gap-2.5 flex-wrap">
              <select
                value={searchField}
                onChange={(e) => setSearchField(e.target.value)}
                className="px-3 py-2 border border-slate-200 rounded-xl text-sm bg-white focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all duration-200"
              >
              ...
              <div className="relative flex-1 min-w-[180px]">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-xl text-sm bg-white focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all duration-200"
                />
              </div>
            </div>
            {list?.is_system && (
              ...
              {selectedIds.size > 0 && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-teal-500 to-teal-600 text-white rounded-xl hover:from-teal-600 hover:to-teal-700 shadow-sm hover:shadow-md transition-all duration-200 hover:scale-[1.03] active:scale-[0.97] text-sm font-medium"
                >
                  <Download size={16} />
          ...
          <table className="w-full">
              ...
              <tr className="bg-slate-50/80 border-b border-slate-200">
                {list?.is_system && (
                  <th className="w-10 px-3 py-4">
                    <button onClick={toggleSelectAll} className="text-slate-400 hover:text-cyan-600 transition-colors duration-200">
                ...
                {list?.columns_config.filter(c => RECORD_COLUMNS.includes(c.key)).map((col) => (
                  <th key={col.key} className="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                    {col.label}
                  </th>
                ))}
            ...
            <tbody>
              {records.map((record, idx) => (
                <tr key={record.id} className={`border-b border-slate-50 transition-all duration-150 hover:bg-cyan-50/30 ${selectedIds.has(record.id) ? 'bg-teal-50/50' : idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                  {list?.is_system && (
                    <td className="w-10 px-3 py-4">
                      <button onClick={() => toggleSelect(record.id)} className="text-slate-300 hover:text-teal-600 transition-colors duration-200">
                    ...
                    <td className="px-6 py-4 text-right">
                      {list?.is_system && (
                        <button
                          ...
                          className="p-1.5 hover:bg-emerald-100 rounded-lg transition-all duration-200 hover:scale-110 active:scale-95"
                          title="Exportar"
                        >
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl px-5 py-3 w-[95vw] max-w-5xl flex flex-col shadow-2xl">
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button onClick={handleSaveRecord} className="px-4 py-2 text-sm bg-gradient-to-r from-cyan-600 to-blue-600 text-white rounded-xl hover:from-cyan-700 hover:to-blue-700 shadow-sm hover:shadow-md transition-all duration-200 font-medium">
